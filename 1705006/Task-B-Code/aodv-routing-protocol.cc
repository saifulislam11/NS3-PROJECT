...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  //-----------------added for r-aodv------------//
    ...
    fprintf(stdout," >>>> Using Randomized AODV\n");
    //check whether to forward or not
    static bool seeded = false;	
    if(!seeded) seeded = true,	
    srand(time(NULL));	
    float num = (float) rand();	
    float random_value = (num / RAND_MAX);	
    float drop_factor = static_cast<float>((1 / (rreqHeader.GetHopCount () + 1.0)));	
    fprintf(stdout,"HopCount : %d\n", rreqHeader.GetHopCount ());	
